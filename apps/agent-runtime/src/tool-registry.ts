...
const definitions: GovernedToolDefinition[] = [
  {
    name: "skill.load",
    label: "Load Skill",
    description: "Load one exact enabled Skill version by the loadRef exposed in available_skills after OS scope, digest, lifecycle, and permission checks.",
    parameters: Type.Object({ load_ref: Type.String({ maxLength: 256 }) }, { additionalProperties: false }),
    risk: "read",
  ...
  },
  {
    ...
    description: "Search bounded public and current-Project-bound Shennong DB Resource metadata using cohort, disease, assay, or modality terms. If a specific biological query has no metadata match, retry with broad cohort or assay terms; personal-chat discovery may return the bounded public catalog as a fallback.",
