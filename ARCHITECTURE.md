...
本文是已发布 V1 的架构、信任边界和验收契约，不是未来路线图。当前实现定位以本仓库的
`apps/server/src/lib.rs`、`apps/agent-runtime/src/server.ts`、`apps/web/app/api/`、
`openapi/os-api.yaml` 和 `deploy/compose.yaml` 为准；跨仓行为必须同时满足对应仓库的
版本化 API。文档与实现不一致时应在同一个变更中修复并记录到 `CHANGELOG.md`，不能只
让其中一方静默漂移。
...
| 架构职责 | 当前实现入口 | 默认三镜像部署 |
| --- | --- | --- |
| 产品 WebUI 与浏览器 BFF | `apps/web`、`apps/web/app/api/v1/[...path]/route.ts`、`apps/web/app/api/agent/route.ts` | `zerostwo/shennong-os` 内的 Web 进程 |
| 身份、RBAC、持久状态、跨服务授权与代理 | `apps/server/src/lib.rs`、`apps/server/src/handlers/` | `zerostwo/shennong-os` 内的 server 进程 |
| Pi Agent loop、prompt/taint/tool/validator 策略 | `apps/agent-runtime/src/server.ts`、`harness.ts`、`tool-policy.ts` | `zerostwo/shennong-os` 内的 Agent 进程 |
| OS 事实数据库 | `migrations/`、Rust `sqlx` 查询 | `zerostwo/shennong-os` 内的 PostgreSQL 进程和 `/data` |
| 产品与 IDE host 路由隔离 | `deploy/container/Caddyfile` | `zerostwo/shennong-os` 内的 Caddy 进程 |
| 跨仓部署契约 | `deploy/compose.yaml`、`deploy/README.md` | `shennong-db`、`runtime` 及以上 OS 服务 |

单个 OS 镜像只是默认分发和进程监管边界，不改变上述逻辑信任边界。Web/BFF 仍不能获得
service secret，Agent Runtime 仍不能直连 DB/Runtime，OS server 仍不能获得 Docker
socket。保留的 hardened profile 可以把这些进程拆成独立服务；协议与状态所有权不变。

### 2.2 跨仓契约与演进规则

| 调用方 → 服务方 | 契约与身份 | OS 侧不变量 |
| --- | --- | --- |
| Web/BFF → OS server | `openapi/os-api.yaml`；用户 session、Origin、CSRF | BFF 只转发 allowlist 路由，不注入 service secret |
| OS server → Agent Runtime | AG-UI `RunAgentInput` + SSE；独立 Agent runtime secret | OS 先创建并授权 Run；浏览器传入的 tools/state/context 不可信 |
| Agent Runtime → OS server | `/api/v1/agent/runs/*` callbacks；独立 OS service token | 所有持久化、capability 校验和后端工具执行都回到 OS |
| OS server → ShennongDB | headless 数据 API；DB service key | OS 先做 Project RBAC 与 shadow sync，只代理精确 path/method allowlist |
| OS server → Shennong Runtime | Runtime V1 API；OS 签发短时 Ed25519 JWT | claim 绑定用户、Project、scope/profile；OS 不接受任意镜像、宿主路径或 Docker 配置 |

三个仓库独立使用 SemVer。破坏跨仓契约的变更必须先提供兼容窗口或联合升级说明，并在
各自 `CHANGELOG.md` 的 `Unreleased` 中记录；`deploy/versions.env`/镜像 digest 是部署时
的组合版本事实，不把三个仓库伪装成一个共享版本号。
...
sequenceDiagram
    ...
    B->>W: POST /api/agent (cookie, Origin, CSRF)
    W->>O: Forward browser identity, no service secret
    O->>P: Authorize Thread/Project and create Run
    O->>A: Authorized RunAgentInput + runtime secret
    A->>O: Bootstrap via OS service token
    ...
    O->>D: Minimum DB credential or scoped Runtime JWT
    D-->>O: Result / EvidenceRef
...
以下 10.1–10.3 是不互信用户和公开服务必须满足的 hardened Runtime 边界。默认
three-image quick deployment 的 `simple` 模式把宿主 Docker socket 只挂给 Runtime，
但该 socket 等同宿主级控制权，因此只能用于可信单用户主机，不能宣称通过这些 hardened
验收项。
...
| 状态 | 唯一事实来源 | 可缓存/映射的位置 | 禁止做法 |
| --- | --- | --- | --- |
| 用户、会话、邀请、Project membership/RBAC | OS PostgreSQL | Web 可持有短期展示缓存 | DB shadow 或客户端 role 参与授权 |
| Thread、Message、Run、event cursor、approval、Memory、Skill enablement | OS PostgreSQL | assistant-ui 可重放，不拥有终态 | 只把运行中状态留在浏览器或 Agent Runtime 内存 |
| Resource、revision、DB Artifact、Relation、Research Graph、provenance | ShennongDB | OS 保存引用和 Project 绑定 | OS 复制大对象后成为第二事实源 |
| Job/Session 执行状态与重启 journal | Runtime SQLite | OS 保存用户可见映射、审计和 Artifact 引用 | Agent Runtime 或 Web 直接控制容器 |
| Project workspace 与工作负载容器 | Runtime 管理的 volume/container；hardened profile 使用专用 rootless daemon | Runtime 根据 server-side profile 解析 | 客户端指定宿主 bind、镜像或 Docker HostConfig |

## 12. 部署拓扑

V1 有两个明确的部署 profile，不能混淆其安全保证：

| Profile | 长期容器 | Docker 边界 | 使用范围 |
| --- | --- | --- | --- |
| 默认 three-image quick deployment | `shennong-os`、`shennong-db`、`shennong-runtime` | Runtime 独占挂载宿主 Docker socket，但该 socket 等同宿主级控制权 | 可信单用户主机、快速安装；不是 hardened 多用户生产边界 |
| retained hardened rootless profile | OS 逻辑进程、headless DB、Runtime control plane；可拆成独立服务 | 只有 Runtime 连接专用 rootless daemon；workload 无宿主 socket | 不互信用户、公开服务和正式验收 |

默认 `zerostwo/shennong-os` 镜像内运行 Caddy、Next.js Web、Rust OS server、Agent
Runtime 和本地 PostgreSQL，分别监听 loopback；只由 Caddy 发布产品/IDE gateway 端口。
`deploy/compose.yaml` 只启动三个仓库镜像，共享 `/config` 中自动生成的服务凭据和 Runtime
Ed25519 公钥；OS 独占私钥。持久状态位于 `./shennong-data/{config,os,db,runtime}`，详见
`deploy/SIMPLE.md`。

hardened profile 的不可信 Project workspace 使用专用 rootless Docker daemon 中带
Runtime/Project 标签的 named volume；该 daemon 的 data-root 位于独立硬上限文件系统，
不把宿主任意路径作为 workload bind mount。生产端口只发布 WebUI/IDE gateway；DB、
Runtime control API、PostgreSQL 和原始 IDE target 不发布。具体主机、密钥和恢复约束见
`deploy/README.md`。

两个 profile 都要求三个仓库独立版本和 digest，不使用一个 `SHENNONG_VERSION` 同步覆盖
三者。`versions.env` 是组合部署和回滚事实；tag 不能代替不可变镜像 digest。

两个 profile 的部署脚本都必须：

1. 检查目录 owner/mode 和磁盘；
2. 生成缺失 secret，不覆盖已有 secret；quick profile 由 OS entrypoint 完成，hardened
   profile 由部署者以 root-owned 文件准备；
3. 构建或拉取 digest-pinned 镜像；
4. 运行数据库 migration；
5. 按依赖顺序启动服务；
6. 完成健康、注册和浏览器 smoke test；
7. 把一次性 bootstrap token 的路径告知部署者，不输出到容器日志。

hardened profile 还必须安装并验证专用 rootless daemon、subuid/subgid、linger 和宿主
egress policy，并完成所有隔离反向测试。quick profile 必须清楚显示 host-socket 风险，
不得把容器健康冒充成 hardened 隔离验收。
...
| 失败 | 对用户的语义 | 恢复与安全要求 |
| --- | --- | --- |
| OS PostgreSQL 不可用 | 控制面 fail closed；登录、RBAC、Run 和写操作不可继续 | 不降级为匿名或客户端授权；恢复后执行 migration/一致性检查 |
| Agent Runtime 或 Provider 不可用 | Run 以结构化错误结束，不能伪造成功结果 | 已持久化历史和 cursor 仍可读取；重试创建新 Run 或受控 child Run |
| ShennongDB 不可用 | 目录、上传、图与 provenance 操作失败；OS 身份状态仍可用 | Project shadow 下次数据面调用前幂等自愈；不绕过 service key/allowlist |
| Runtime/Docker backend 不可用 | 新 Job/Session 失败，进行中的实例标记 failed/lost | OS 不接管 Docker socket；Runtime journal 对账后再允许重试 |
| 浏览器/SSE 断线 | 不重新运行模型 | 按最后 cursor replay/follow，重复边界事件去重 |
| IDE origin 或 launch ticket 异常 | 拒绝访问，产品会话保持隔离 | ticket 一次性、短时、日志脱敏；IDE host 普通 OS API 始终 404 |
| 跨仓版本不兼容 | readiness/验收失败并阻止发布 | 使用独立版本/digest 回滚，禁止跳过契约门禁强行启动 |
